export function render () {
  ...
  function animateDots () {
    ...
    drawDots()
    moveMousePosition()
    if(dots.nb!=canvas.width/2){return}
    requestAnimationFrame(animateDots)
  }

  var positionVx=0.3,positionVy=0.3;

  function moveMousePosition () {

    if (mousePosition.y < 0 || mousePosition.y > canvas.height) {
      
      positionVx = positionVx
      positionVy = -positionVy
    } else if (mousePosition.x < 0 || mousePosition.x > canvas.width) {
      positionVx = -positionVx
      positionVy = positionVy
    }
    mousePosition.x += positionVx
    mousePosition.y += positionVy
  }
